...
;; TODO: While entering the same buffer there was Error processing message (wrong-type-argument window-live-p #<window num>). What cauesd this error?
...
(defun tlwm-emacs-windows ()
  (window-list nil 'never-include-minibuffer (frame-first-window)))
...
(defun tlwm-update-layout ()
  "Update layout and printit in minibuffer."
  (funcall (nth 0 tlwm-layouts))
  (tlwm-display-status)
  )
...
(defun tlwm-global-enable ()
  ...
  (add-hook 'window-state-change-hook 'tlwm-state-update))

(defun tlwm-global-disable ()
  (remove-hook 'window-state-change-hook 'tlwm-state-update))
